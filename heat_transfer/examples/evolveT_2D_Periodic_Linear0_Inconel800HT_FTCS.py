...
'''
def evolveT_2D_Periodic_Linear0_Inconel800HT_FTCS():
  st = time.time()
  directory = "../../data/FTCS_2D_linear0_Periodic_Inconel800HT_T.txt"
  ...
  t, x, T, mask, _lambda = IC_2D_Linear0_Inconel800HT()

  for i in range(len(t)):
    ti = t[i]
    print("t=" + str(ti) + "s; t_max=" + str(t_max))
    # making boundary conditions
    T = BC_2D_Periodic_Dirichlet(T, x, mask)
    print(np.array_str(T, precision=3, suppress_small=True))
...
duration = evolveT_2D_Periodic_Linear0_Inconel800HT_FTCS()

print("finished evolveT_2D_Periodic_Linear0_Inconel800HT_FTCS() in " + str(duration) +"s.")
